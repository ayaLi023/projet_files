     ...
     while (!WindowShouldClose()) {
        ...
        for (int key = KEY_ZERO; key <= KEY_NINE; key++) {
            ...
            // Afficher le résultat de la recherche
                if (position != -1) {
                    DrawText(TextFormat ("La valeur %d existe dans la file!",valeurRecherchee), 20, 300, 20.5, GREEN);
                } else {
                    DrawText(TextFormat("La valeur %d n'existe pas dans la file!",valeurRecherchee), 20, 300, 20.5, RED);
                }
